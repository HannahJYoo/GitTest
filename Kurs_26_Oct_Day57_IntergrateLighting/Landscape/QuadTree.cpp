...
{
	material = new Material(Shaders + L"020_Terrain.hlsl");
...
void QuadTree::Render()
{
	...
	if (bFrustum) frustum->LineRender();
	
	RenderNode(parent);


	ImGui::Text("QuadTree Draw : %d", drawCount);
	ImGui::Checkbox("Seh Frustum", &bFrustum); ImGui::SameLine();
	ImGui::Checkbox("Seh TreeLevel", &bTreeLevel);
	ImGui::Separator();
	ImGui::SliderInt("GridLine", &lineBuffer->Data.Type, 0, 2);
	ImGui::SliderInt("Spacing", &lineBuffer->Data.Spacing, 1, 20);
}
...
void QuadTree::RenderNode(NodeType * node)
{
	...
	if (bTreeLevel) {
		if (node->Width < parent->Width) {
			D3DXVECTOR3 minPos, maxPos, lCenter(0, 5.0f, 0);
			D3DXCOLOR lColor(1, 1, 1, 1);
			float d = node->Width / 2.0f;
			UINT dlevel = (UINT)parent->Width, level = 0;

			while ((level < 10) && (node->Width < dlevel)) {
				level++;
				dlevel = ((UINT)parent->Width) >> level;
			}
			...
			minPos = D3DXVECTOR3(lCenter.x - d, -lCenter.y, lCenter.z - d) + center;
			maxPos = D3DXVECTOR3(lCenter.x + d, lCenter.y, lCenter.z + d) + center;

			gline->SetColor(lColor);
			gline->DrawBox(minPos, maxPos);
			gline->Render();
		}
	}
