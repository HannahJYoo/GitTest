...
Water::Water(ExecuteValues * values, UINT width, UINT height, float thick, Texture * heightMap)
	:values(values), width(width), height(height), thick(thick), heightMap(heightMap), bWireframe(false), bBlend(true)
{
	material = new Material(Shaders + L"Landscape_Water.hlsl");
...
void Water::Render()
{
	...
	ImGui::Begin("Setting Water");
	{
		ImGui::Checkbox("WireFrame", &bWireframe); ImGui::SameLine();
		ImGui::Checkbox("Blend", &bBlend);
		
		
		ImGui::ColorEdit4("ShallowColor", psBuffer->Data.ShallowColor);
		ImGui::ColorEdit4("DeepColor", psBuffer->Data.DeepColor);
		
		ImGui::SliderFloat("Bias", &psBuffer->Data.Bias, 0.0f, 2.0f);
		ImGui::SliderFloat("Power", &psBuffer->Data.Power, 0.0f, 2.0f);
		ImGui::SliderFloat("Amount", &psBuffer->Data.Amount, 0.0f, 2.0f);
		ImGui::SliderFloat("HeightRatio", &psBuffer->Data.HeightRatio, 0.0f, 20.0f);
		ImGui::SliderFloat("ShoreBlend", &psBuffer->Data.ShoreBlend, 0.0f, 100.0f);
		ImGui::SliderFloat("WaveFrequency", &vsBuffer->Data.WaveFrequency, 0.0f, 0.3f);
		ImGui::SliderFloat("WaveAmplitude", &vsBuffer->Data.WaveAmplitude, -2.0f, 2.0f);

		ImGui::SliderFloat2("TextureScale", vsBuffer->Data.TextureScale, 5.0f, 50.0f);
		ImGui::SliderFloat2("BumpSpeed", vsBuffer->Data.BumpSpeed, -0.5f, 0.5f);
		ImGui::SliderFloat("BumpHeight", &vsBuffer->Data.BumpHeight, 0.0f, 5.0f);
