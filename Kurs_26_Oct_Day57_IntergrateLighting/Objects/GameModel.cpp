...
D3DXCOLOR * GameModel::GetDiffuse() {
	D3DXCOLOR* color;
	for (Material* material : model->Materials())
...
void GameModel::SetSpecular(D3DXCOLOR & color)
{
 ...
 D3DXCOLOR * GameModel::GetSpecular() {
	D3DXCOLOR* color;
	for (Material* material : model->Materials())
...
bool GameModel::bModelPick(D3DXVECTOR3 & origin, D3DXVECTOR3 & direction, OUT D3DXVECTOR3 & position)
{
	float u, v, distance;
